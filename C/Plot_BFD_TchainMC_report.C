...
void plotVariables2() {
    ...
    TH1D* hist_signal = new TH1D("hist_total", "B_FD_OWNPV distribution for MC TChain", 100, -10, 150);
    TH1D* hist_bkg = new TH1D("hist_filtre_bkg", "B_FD_OWNPV distribution for BKG=10", 100, -10, 150);
   
    hist_signal->GetXaxis()->SetTitle("B_FD_OWNPV distribution (mm)");
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        tree->GetEntry(i);
        if (B_BKGCAT == 10) {
            hist_signal->Fill(B_FD_OWNPV);
        } else if (B_BKGCAT != 10) {
            hist_bkg->Fill(B_FD_OWNPV);
        }
    }
