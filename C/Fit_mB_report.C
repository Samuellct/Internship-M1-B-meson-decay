...
void plotVariables2() {
    ...
    TH1D* hist_mass_inv_B_sig = new TH1D("hist_mass_inv_B", "mass inv de B", 100, 4300, 7200);
    TH1D* hist_mass_inv_B_data = new TH1D("hist_mass_inv_B", "mass inv de B", 100, 4300, 7200);
    ...
    for (Long64_t i = 0; i < nentries1; ++i) {
        ...
        if (B_BKGCAT == 10) {
            ...
            // OTHER CUTS
            bool otherCuts = piminus_ProbNNpi > 0.2 && piminus_ProbNNk < 0.8 && piminus_ProbNNp < 0.8 &&
                piplus_ProbNNpi > 0.2 && piplus_ProbNNk < 0.8 && piplus_ProbNNp < 0.8 &&
                Kplus_ProbNNpi < 0.8 && Kplus_ProbNNk > 0.2 && Kplus_ProbNNp < 0.8 &&
                gamma_CL > 0.2 && gamma_PP_IsPhoton > 0.5 && gamma0_CL > 0.2 && gamma0_PP_IsPhoton > 0.5 &&
                gamma1_CL > 0.2 && gamma1_PP_IsPhoton > 0.5;
            // FILL TRIGGER HISTOGRAMS
            if (L0_trigger && HLT1 && HLT2 && massCuts && special && otherCuts) {
    ...
    for (Long64_t i = 0; i < nentries2; ++i) {
        ...
        // OTHER CUTS
        bool otherCuts_data = piminus_ProbNNpi_data > 0.2 && piminus_ProbNNk_data < 0.8 && piminus_ProbNNp_data < 0.8 &&
            piplus_ProbNNpi_data > 0.2 && piplus_ProbNNk_data < 0.8 && piplus_ProbNNp_data < 0.8 &&
            Kplus_ProbNNpi_data < 0.8 && Kplus_ProbNNk_data > 0.2 && Kplus_ProbNNp_data < 0.8 &&
            gamma_CL_data > 0.2 && gamma_PP_IsPhoton_data > 0.5 && gamma0_CL_data > 0.2 && gamma0_PP_IsPhoton_data > 0.5 &&
            gamma1_CL_data > 0.2 && gamma1_PP_IsPhoton_data > 0.5;
        // FILL TRIGGER HISTOGRAMS
        if (L0_data && HLT1_data && HLT2_data && massCuts_data && special_data && otherCuts_data) {
    ...
    legend->AddEntry(hist_mass_inv_B_sig, "MC", "l");
    legend->AddEntry(hist_mass_inv_B_data, "Data", "l");
    ...
    //hist_mass_inv_B_sig->Draw();
    hist_mass_inv_B_data->Draw();
