...
void plotVariables2() {
    ...
    TH1D* hist_signal = new TH1D("hist_total", "gamma0_CL distribution for MC TChain", 100, -0.1, 1.1);
    TH1D* hist_bkg = new TH1D("hist_filtre_bkg", "gamma0_CL distribution for BKG=10", 100, -0.1, 1.1);
   
    hist_signal->GetXaxis()->SetTitle("gamma0_CL distribution");
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        tree->GetEntry(i);
        if (B_BKGCAT == 10) {
            hist_signal->Fill(gamma0_CL);
        } else if (B_BKGCAT != 10) {
            hist_bkg->Fill(gamma0_CL);
        }
    }
