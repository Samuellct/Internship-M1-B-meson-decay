...
void plotVariables2() {
    ...
    TH1D* hist_mK1_MC = new TH1D("hist_mK1_MC", "", 100, 1000, 1600);
    TH1D* hist_mK1_data = new TH1D("hist_mK1_data", "", 100, 1000, 1600);

    TH1D* hist_momega_MC = new TH1D("hist_momega_MC", "", 100, 400, 1100);
    TH1D* hist_momega_data = new TH1D("hist_momega_data", "", 100, 400, 1100);
   
   TH1D* histograms[] = { hist_mK1_MC, hist_momega_MC};
    ...
    for (Long64_t i = 0; i < nentries2; ++i) {
        ...
        // OTHER CUTS
        bool ChargedPIDcut = piminus_ProbNNpi_data > 0.2 && piminus_ProbNNk_data < 0.8 && piminus_ProbNNp_data < 0.8 && piplus_ProbNNpi_data > 0.2 && piplus_ProbNNk_data < 0.8 && piplus_ProbNNp_data < 0.8 && Kplus_ProbNNpi_data < 0.8 && Kplus_ProbNNk_data > 0.2 && Kplus_ProbNNp_data < 0.8;
        bool NeutralPIDcut = gamma_CL_data > 0.2 && gamma_PP_IsPhoton_data > 0.5 && gamma0_CL_data > 0.2 && gamma0_PP_IsPhoton_data > 0.5 && gamma1_CL_data > 0.2 && gamma1_PP_IsPhoton_data > 0.5;
        // FILL HISTOGRAMS
        if (massCuts_data && L0_data && HLT1_data && HLT2_data && ChargedPIDcut && NeutralPIDcut) {
            hist_mK1_data->Fill(B_M01234_data);
    ...
    legend1->AddEntry(hist_mK1_MC, "MC events", "l");
    legend1->AddEntry(hist_mK1_data, "Data events", "l");
    ...
    plot_canvas->cd(1);
    gPad->SetLogy(1);
    hist_mK1_MC->SetLineColor(kRed);
    hist_mK1_MC->DrawNormalized();
    hist_mK1_data->SetLineColor(kBlue);
    hist_mK1_data->DrawNormalized("same");
    legend1->Draw("same");
    ...
    hist_momega_data->DrawNormalized("same");
    legend2->Draw("same");  
